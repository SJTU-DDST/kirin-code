...
static void ssd_init_lines(struct ssd *ssd)
{
    ...
    lm->lines = linear_malloc(sizeof(struct line) * lm->tt_lines, 0);
...
{
    size_t flash_size = (1.0 + CONFIG_NAND_OP_RATIO) * SSD_SIZE;
...
static void ssd_init_nand_blk(struct nand_block *blk, struct ssdparams *spp)
{
    ...
    blk->pg = linear_malloc(sizeof(struct nand_page) * blk->npgs, 0);
...
static void ssd_init_nand_plane(struct nand_plane *pl, struct ssdparams *spp)
{
    ...
    pl->blk = linear_malloc(sizeof(struct nand_block) * pl->nblks, 0);
...
static void ssd_init_nand_lun(struct nand_lun *lun, struct ssdparams *spp)
{
    ...
    lun->pl = linear_malloc(sizeof(struct nand_plane) * lun->npls, 0);
...
static void ssd_init_ch(struct ssd_channel *ch, struct ssdparams *spp)
{
    ...
    ch->lun = linear_malloc(sizeof(struct nand_lun) * ch->nluns, 0);
...
static void ssd_init_maptbl(struct ssd *ssd)
{
    ...
    ssd->maptbl = linear_malloc(sizeof(struct ppa) * spp->tt_pgs, 0);
...
static void ssd_init_rmap(struct ssd *ssd)
{
    ...
    ssd->rmap = linear_malloc(sizeof(uint64_t) * spp->tt_pgs, 0);
    for (int i = 0; i < spp->tt_pgs; i++) {
...
void ssd_init(struct ssd *ssd)
{
    ...
    ssd->ch = linear_malloc(sizeof(struct ssd_channel) * spp->nchs, 0);
