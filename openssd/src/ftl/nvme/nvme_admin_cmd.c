...
void handle_fs_status(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
...
void handle_get_address(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL)
{
	unsigned int table_num = nvmeAdminCmd->dword10;
	uint64_t table_id = nvmeAdminCmd->dword11;
	table_id = (table_id << 32) + nvmeAdminCmd->dword12;
	unsigned int is_args_file = nvmeAdminCmd->dword13;
	unsigned int is_result_file = nvmeAdminCmd->dword14;
	nvmeCPL->dword[0] = 0;
	nvmeCPL->specific = 0x0;
	
	get_file_address(table_id, table_num, is_args_file, is_result_file);
}
...
void handle_nvme_admin_cmd(NVME_COMMAND *nvmeCmd)
{
	...
	switch(opc)
	{
		...
		{
			handle_fs_status(nvmeAdminCmd, &nvmeCPL);
			break;
		}
		...
		{
			handle_get_address(nvmeAdminCmd, &nvmeCPL);
			break;
		}
		case ADMIN_RESET_CS_STATUS:
		{
			reset_cs_status();
			break;
		}
